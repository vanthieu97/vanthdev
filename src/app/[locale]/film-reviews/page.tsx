...
import { getCanonicalUrl, getLocalizedPath, LOCALES, isValidLocale, type Locale } from '@/lib/i18n/config';
...
function getPageUrl(locale: Locale) {
  return getCanonicalUrl(baseUrl, locale, '/film-reviews');
}
...
function buildItemListJsonLd(locale: string) {
  return {
    ...
    itemListElement: REVIEWS.map((r, i) => ({
      ...
      item: {
        ...
        description: r.desc,
        url: getCanonicalUrl(baseUrl, locale as Locale, `/film-reviews/${r.slug}`),
        image: r.youtubeId
  ...
  return (
    <div className="bg-[#faf8f5] dark:bg-[#0a0f1a]">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(buildItemListJsonLd(locale)) }}
      />
        ...
        <ul className="space-y-4" role="list">
          {REVIEWS.map((r) => {
            ...
            return (
              <li key={slug}>
                <Link
                  href={getLocalizedPath(locale, `/film-reviews/${slug}`)}
