...
import type { Locale as ConfigLocale } from '@/lib/i18n/config';

const TZ_VIETNAM = 'Asia/Ho_Chi_Minh';

function formatDate(dateStr: string, locale: ConfigLocale): string {
  try {
    ...
    <>
      ...
      {nextPage && (
        ...
        >
          ...
          {loading ? (
            <p className="text-[#8a8a8a] dark:text-slate-500 text-sm">{t.loading}</p>
          ) : (
            <button
              type="button"
